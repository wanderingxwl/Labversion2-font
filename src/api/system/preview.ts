...
// };
export const previewFile = (id: number, fileType: number) => {
  switch (fileType) {
    case 0:
      //blob
      return http.request("get", `/preview/${id}`, {
        responseType: "blob"
      });
    case 1:
      return http.request("get", `/preview/${id}`, {
    ...
    case 2:
      return http.get(`/preview/${id}`);
  }
};
export const previewImage = (id: number) => {
  return (
    http.request("get", `/previewImage/${id}`),
    {
      responseType: "arraybuffer"
    }
  );
